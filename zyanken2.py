...
'''
* 本スクリプトの処理の内容
本スクリプトは、ディープラーニングでじゃんけんを学習する処理が記述されたスクリプト。
...
BATTLE_RESULT_SET = [EVEN, WIN, LOSE, LOSE, EVEN, WIN, WIN, LOSE, EVEN]
...
def get_training_data():
    ...
    label = np.array(BATTLE_RESULT_SET).repeat(REPEAT_CNT)

    # * to_categoricalメソッド
    # 説明    : もともと 0 or 1 or 2 である勝敗データを以下の形式に変換する。（いわゆるone hot表現）
...
def get_neural_network_model():
    ...
    model = Sequential()
    model.add(Dense(300, activation='relu', input_dim=2)) # 今回の入力値は自分の手と相手の手の2種類なので入力値を2つに指定。
...
def print_correct_answer_rate(model) :
    ...
    print("*----- 結果 -----*")
    print("自分　 vs 相手　 => AIの判定結果 : 予想は当たった？")

    # zip関数
    # 説明    : pythonの標準関数。複数のリスト(配列)を同時にイテレートできる。
    # 参考資料 : https://note.nkmk.me/python-zip-usage-for/
    for hands, real in zip(HAND_SET, BATTLE_RESULT_SET):
      predict = model.predict(np.array([[hands[0], hands[1]]])) # 自分と相手の手を入力してモデルに結果を予想してもらう。

      predict_result = np.argmax(predict) # 予想結果の数値を取得。(引分 : 0, 勝利 : 1, 敗北 : 2)
      if predict_result == real:
        correct += 1
      print(JP_HAND_NAMES[hands[0]] + ' vs ' + JP_HAND_NAMES[hands[1]] + ' => ' + JP_RESULT_NAMES[predict_result] + ' : ' + str(predict_result == real))

      # モデルが勝率をどのように予測したのかの内訳を出力
      if PRINT_DETAIL_RATE:
        print(' ↑予想確率詳細 : 引分->%02.2f' % predict[0][0] + ' 勝利->%02.2f' % predict[0][1] + ' 敗北->%02.2f' % predict[0][2])
